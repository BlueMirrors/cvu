"""This file contains Yolo's core IDetector implementation.

Yolo Core represents a common interface over which users can perform
Yolo powered object detection, without having to worry about the details of
...
class Yolo(IDetector):
    """Implements ICore for Yolo

    Yolo Core represents a common interface to perform
    Yolo (v5 & v7) powered object detection.
    """
    _BACKEND_PKG = "cvu.detector.yolo.backends"
    ...
    def __init__(self,
        ...
        if kwargs.get("input_shape", None) is not None:
            self._preprocess = [
                lambda image: letterbox(
                    image, kwargs['input_shape'], auto=False), bgr_to_rgb
            ]
        else:
    ...
    def __call__(self, inputs: np.ndarray, **kwargs) -> Predictions:
        """Performs Yolo Object Detection on given inputs.
    ...
    def _scale(original_shape: Tuple[int], process_shape: Tuple[int],
        ...
        Args:
            ...
            outputs (np.ndarray): outputs from yolo model
    ...
    def _load_model(self, backend_name: str, weight: str, device: str,
        ...
        """
        # load model
        backend = import_module(f".yolo_{backend_name}", self._BACKEND_PKG)

        if backend_name != 'tensorrt':
            self._model = backend.Yolo(weight, device)
        else:
            self._model = backend.Yolo(weight,
    ...
    def _to_preds(self, outputs: np.ndarray) -> Predictions:
        """Convert Yolo's numpy inputs to Predictions object.

        Args:
            outputs (np.ndarray): basic outputs from yolo inference.

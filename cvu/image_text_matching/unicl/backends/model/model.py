...
class UniCLModel(nn.Module):
    ...
    def __init__(
        ...
        self.text_encoder = build_text_encoder(self.conf_lang_encoder,
                                               self.tokenizer,
                                               config['VERBOSE'])
        ...
        if hasattr(self.text_encoder, 'dim_out'):
            ...
            with torch.no_grad():
                dim_out = self.text_encoder(
                    torch.zeros(1, 1).type(
                        torch.LongTensor))['last_hidden_state'].size(2)
    ...
    def from_pretrained(self,
        ...
        self.image_encoder.from_state_dict(image_encoder_state_dict, ['*'],
                                           verbose)
    ...
    if config['MODEL']['PRETRAINED'] != '':
        ...
        if is_valid_url(pretrained_path):
            with tempfile.TemporaryDirectory() as tmp_path:
                ...
                model.from_pretrained(str(file_local_path),
